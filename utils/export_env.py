...
def export_env(history_only=False, include_builds=False, channels=True):
    """Capture `conda env export` output"""
    cmd = ["conda", "env", "export", "-c", "conda-forge", "-c", "pytorch", "-c", "pyg"]
    if history_only:
...
def _combine_env_data(env_data_full, env_data_hist):
    ...
    env_data["dependencies"].append('python=3.9')
