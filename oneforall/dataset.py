...
class DatasetMap():
    @staticmethod
    def duorc(example):
        nr_answer = len(example["answer"])
        return [example["plot"]]*nr_answer, [example["question"]]*nr_answer, [answer if len(answer) > 0 else "" for answer in example["answers"]]

    @staticmethod
    def squad(example):
        return [example["context"], [example["question"]], example["answer"]]


class Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset, tokenizer, parser=None):
        ...
        for idx, row in tqdm(dataset.iterrows()):
            _contexts, _questions, _answers = parser(row.to_dict())
